...
import { CommandButtons, DashboardContentContainer, DashboardTitleBar, DataColumn, FirstPart, FormContainer, SecondPart, ThirdPart } from '../../../components/styled-components/dashboardStyledComponents'
...
const Settings = () => {
  ...
  useEffect(()=>{
    axios.get(`${Apis.userApis.findById}${JSON.parse(localStorage.getItem('admnInfo')).id}`)
    .then(response => {
  ...
  const requestPasswordReset = () => {
    ...
    axios.post(Apis.userApis.requestPasswordReset, {email: user.email, role: 'admin'})
    .then(response => {
      setTimeout(()=>{
        if (response.status === 200 || response.status === 201) {            
